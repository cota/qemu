...
static inline bool is_singlestepping(DisasContext *s)
{
     ...
     */
    return s->b.singlestep_enabled || s->ss_active;
}
...
static inline void gen_bx_excret(DisasContext *s, TCGv_i32 var)
{
     ...
     * s->b.is_jmp that we need to do the rest of the work later.
     */
    gen_bx(s, var);
    if (s->v7m_handler_mode && arm_dc_feature(s, ARM_FEATURE_M)) {
        s->b.is_jmp = DISAS_BX_EXCRET;
    }
}
...
static inline void gen_lookup_tb(DisasContext *s)
{
    ...
    tcg_gen_movi_i32(cpu_R[15], b->pc & ~1);
    b->is_jmp = DISAS_EXIT;
}
...
static inline bool use_goto_tb(DisasContext *s, target_ulong dest)
{
    ...
    return (b->tb->pc & TARGET_PAGE_MASK) == (dest & TARGET_PAGE_MASK) ||
           ((b->pc - 1) & TARGET_PAGE_MASK) == (dest & TARGET_PAGE_MASK);
#else
    return true;
#endif
}
...
static void gen_goto_tb(DisasContext *s, int n, target_ulong dest)
{
    if (use_goto_tb(s, dest)) {
        ...
        tcg_gen_exit_tb((uintptr_t)s->b.tb + n);
    } else {
        gen_set_pc_im(s, dest);
        gen_goto_ptr();
    }
...
static int disas_thumb2_insn(CPUARMState *env, DisasContext *s, uint16_t insn_hw1)
{
    ...
    if (!(arm_dc_feature(s, ARM_FEATURE_THUMB2)
        ...
        if ((b->pc & ~TARGET_PAGE_MASK) == 0) {
            /* Instruction spans a page boundary.  Implement it as two
               16-bit instructions in case the second half causes an
               prefetch abort.  */
            offset = ((int32_t)insn << 21) >> 9;
            tcg_gen_movi_i32(cpu_R[14], b->pc + 2 + offset);
            return 0;
        }
        /* Fall through to 32-bit decode.  */
    }

    insn = arm_lduw_code(env, b->pc, s->sctlr_b);
    b->pc += 2;
    ...
    case 8: case 9: case 10: case 11:
        if (insn & (1 << 15)) {
            ...
            if (insn & 0x5000) {
                ...
                if (insn & (1 << 14)) {
                    /* Branch and link.  */
                    tcg_gen_movi_i32(cpu_R[14], b->pc | 1);
                }
    ...
    case 12: /* Load/store single data item.  */
        ...
        if (rn == 15) {
            addr = tcg_temp_new_i32();
            /* PC relative.  */
            /* b->pc has already been incremented by 4.  */
            imm = b->pc & 0xfffffffc;
            if (insn & (1 << 23))
                imm += insn & 0xfff;
            else
...
static void disas_thumb_insn(CPUARMState *env, DisasContext *s)
{
    ...
    case 10:
        ...
        if (insn & (1 << 11)) {
            ...
            /* PC. bit 1 is ignored.  */
            tmp = tcg_temp_new_i32();
            tcg_gen_movi_i32(tmp, (b->pc + 2) & ~(uint32_t)2);
        }
        val = (insn & 0xff) * 4;
...
static void arm_tr_init_context(DisasBase *db, CPUARMState *env)
{
    ...
    dc->next_page_start = (dc->pc_start & TARGET_PAGE_MASK) + TARGET_PAGE_SIZE;
}
...
static void arm_tr_tb_start(DisasBase *db, CPUARMState *env)
{
    DisasContext *dc = container_of(db, DisasContext, b);
...
arm_tr_breakpoint_do(DisasBase *db, CPUARMState *env, const CPUBreakpoint *bp)
{
    DisasContext *dc = container_of(db, DisasContext, b);

    if (bp->flags & BP_CPU) {
        gen_set_condexec(dc);
        gen_set_pc_im(dc, db->pc);
        gen_helper_check_breakpoints(cpu_env);
        /* End the TB early; it's likely not going to be executed */
        db->is_jmp = DISAS_UPDATE;
        return BREAKPOINT_CONT_INSN;
    } else {
    ...
    do {
        ...
        if (unlikely(!QTAILQ_EMPTY(&cpu->breakpoints))) {
            ...
            QTAILQ_FOREACH(bp, &cpu->breakpoints, entry) {
                if (bp->pc == db->pc) {
                    BreakpointAction ba = arm_tr_breakpoint_do(db, env, bp);

                    if (ba == BREAKPOINT_STOP_TB) {
         ...
         */
        end_of_page = (db->pc >= dc->next_page_start) ||
            ((db->pc >= dc->next_page_start - 3) && insn_crosses_page(env, dc));
...
done_generating:
    ...
    if (qemu_loglevel_mask(CPU_LOG_TB_IN_ASM) &&
        qemu_log_in_addr_range(dc->pc_start)) {
        qemu_log_lock();
        qemu_log("----------------\n");
        qemu_log("IN: %s\n", lookup_symbol(dc->pc_start));
        log_target_disas(cpu, dc->pc_start, db->pc - dc->pc_start,
                         dc->thumb | (dc->sctlr_b << 1));
        qemu_log("\n");
        qemu_log_unlock();
    }
#endif
    tb->size = db->pc - dc->pc_start;
