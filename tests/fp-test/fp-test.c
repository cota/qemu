...
static enum error soft_tester(struct test_op *t)
{
    ...
    if (t->prec == PREC_FLOAT) {
        ...
        case OP_MULADD:
            ...
            break;
        case OP_MULADD_NEG_ADDEND:
            res = float64_muladd(a, b, c, float_muladd_negate_c, s);
            break;
        case OP_MULADD_NEG_PRODUCT:
            res = float64_muladd(a, b, c, float_muladd_negate_product, s);
            break;
        case OP_MULADD_NEG_RESULT:
            res = float64_muladd(a, b, c, float_muladd_negate_result, s);
            break;
        case OP_DIV:
